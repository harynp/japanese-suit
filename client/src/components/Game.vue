  ...
  export default {
    ...
    methods: {
      ...
      checkRoom () {
        ...
        this.$db.ref('/JAPAN/room/').once('value', function (snapshot) {
          var count = 0
      ...
      gameStarted () {
        ...
        if (this.Winner === 'Player 1 Win') {
          this.$db.ref(`/JAPAN/` + room + '/' + id).child('winner').set(this.Winner)
        } else if (this.Winner === 'Player 2 Win') {
          this.$db.ref(`/JAPAN/` + room + '/' + id).child('winner').set(this.Winner)
        } else {
          console.log('SERI')
        }
